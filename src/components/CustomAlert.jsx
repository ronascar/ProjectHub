...
export default function CustomAlert({ isOpen, onClose, title, message, type = 'success', confirmText = 'OK, Entendido' }) {
            ...
            <div className="flex min-h-full items-center justify-center p-4">
                <div className="relative transform overflow-hidden rounded-2xl bg-white dark:bg-[#1e293b] shadow-2xl transition-all w-full max-w-md">
                    ...
                    <div className="p-8">
                        ...
                        <button
                            onClick={onClose}
                            className={`w-full rounded-xl px-6 py-3 text-base font-semibold text-white shadow-lg transition-all hover:scale-[1.02] focus:outline-none focus:ring-4 ${buttonColors[type]}`}
                        >
                            {confirmText}
                        </button>
                    </div>
...
// Hook para usar o alert customizado
export function useCustomAlert() {
    ...
    const showAlert = ({ title, message, type = 'success', confirmText = 'OK, Entendido' }) => {
        setAlertState({
            isOpen: true,
            title,
            message,
